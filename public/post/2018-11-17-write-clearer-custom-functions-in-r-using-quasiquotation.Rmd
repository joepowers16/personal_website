...
My calling arguments are like genies. Magic, but dangerous to have floating around loose inside of my function. `enquo()` traps the genie in the bottle for safe transport, and `!!` rubs the bottle to let him out and grant my wishes. 
```{r}
double_dplyr <- function(data, x){
  x <- enquo(x)
  data %>% 
...
Naming `new_var` as a string will not help either. Because in this context `mutate()` is not going to look for the calling argument that `new_var` maps to. In this context `mutate()` will just create `new_var` rather than "cyl_2" as the new variable. 
```{r}
double_dplyr <- function(data, x, new_var){
  x <- enquo(x)
  new_var <- enquo(new_var)
...
# Conclusion: 
If you know when to yse `enquo()`and `!!` and you can write reliable functions that are simple to use and understand.    
