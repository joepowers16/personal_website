...
# TLDR
The functions in `dplyr`, `tidyr`, and `ggplot2` handle their arguments differently than most R functions. If you know when to use quasiquotation functions like `enquo()` & `!!`, you can quickly write very powerful custom functions that take full advantage of the functions in `dplyr`, `tidyr`, and `ggplot2`. This post demonstrates standard use cases for quasiquotation functions `enquo()` & `!!`, `enquos()` & `!!!`, `quo_name()` and the helper function `:=`, which is godsend for naming new variables inside of custom functions. 
